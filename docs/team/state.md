...
## schema57：副账号职级与数字员工分配（D-056）

更新时间：2026-08-20
- users.job_title（总监/经理/员工）+ allowed_emp_idxs_json（数字员工白名单，NULL=行业内全部）。
- 老板全权；总监→经理/员工、经理→员工可限域分配（只能分自己行业内、自己可用的员工）。
- 后端强制：楼层过滤、员工详情 403、单独派活 403、会议建会校验+自动选人过滤。
- 团队页改版（职级徽章/分配器/经理受限视图）；15 项权限矩阵测试全绿。
- 计划 r26 发版（迁移 v57 随切流执行）。
